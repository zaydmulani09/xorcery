...
<section class="about">
  ...
  <p>The specs are C expressions over 32-bit words (<code>(int)x &gt;&gt; 31</code> is an arithmetic shift, <code>x &gt;&gt; 31</code> a logical one; <code>popcnt</code>, <code>clz</code>, <code>ctz</code>, <code>mulhi</code>, <code>rotl</code>, <code>select</code> and friends are built in). Constants come from the small pool you choose — plus, for the last instruction, any 32-bit constant at all: <code>x op c</code> is solved for <code>c</code> from the samples instead of enumerated, so a mask like <code>0x55555555</code> does not have to be in the pool. The classic answers in the gallery come from Henry Warren's <em>Hacker's Delight</em>; the first 25 are the standard synthesis benchmark from Gulwani et al. (PLDI 2011).</p>
  <p>Instruction order is canonical (independent instructions are enumerated once), dead code is never generated, every op is self-tested against the CPU on your GPU before a search runs, and a verification verdict is only accepted after the GPU's arithmetic passes a spot check against the CPU — the Intel driver this was built on miscompiles <code>countOneBits</code>, which is how that check came to exist.</p>
  <p>Read the <a href="https://github.com/zaydmulani09/xorcery#readme" target="_blank" rel="noopener">README</a> for how the search kernel works, the pruning rules, the honest limits (2^64 pairs cannot be enumerated), and benchmark numbers. The same engine runs from a terminal on the native GPU via Deno: <code>deno task cli "x &amp; (x - 1)"</code>.</p>
</section>
<footer>
